 ...
 public:
  ...
  void Stop() {
    ...
    Instrumentor::Get().WriteProfile(
        {m_Name, start, end, std::this_thread::get_id()});
...
#define KZ_PROFILE 1
#if KZ_PROFILE
#define KZ_PROFILE_BEGIN_SESSION(name, filepath) \
  ::Kazel::Instrumentor::Get().BeginSession(name, filepath)
#define KZ_PROFILE_END_SESSION() ::Kazel::Instrumentor::Get().EndSession()
#define KZ_PROFILE_SCOPE(name) \
  ::Kazel::InstrumentationTimer timer##__LINE__(name);
#define KZ_PROFILE_FUNCTION() KZ_PROFILE_SCOPE(__FUNCSIG__)
#else
#define KZ_PROFILE_BEGIN_SESSION(name, filepath)
#define KZ_PROFILE_END_SESSION()
#define KZ_PROFILE_SCOPE(name)
#define KZ_PROFILE_FUNCTION()
#endif