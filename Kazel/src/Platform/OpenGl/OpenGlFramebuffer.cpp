...
void OpenGlFramebuffer::Invalidate() {
      ...
      switch (m_ColorAttachmentSpecifications[i].TextureFormat) {
        case FramebufferTextureFormat::RGBA8:
          Utils::AttachColorTexture(
              m_ColorAttachments[i], m_Specification.Samples, GL_RGBA8, GL_RGBA,
  ...
  if (m_ColorAttachments.size() > 1) {
    KZ_CORE_CHECK(m_ColorAttachments.size() <= 4);
...
void OpenGlFramebuffer::Resize(uint32_t width, uint32_t height) {
  if (width == 0 || height == 0 || width > s_MaxFramebufferSize ||
    ...
    KZ_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
    return;
  }
...
int OpenGlFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) {
  KZ_CORE_CHECK(attachmentIndex < m_ColorAttachments.size());
...
void OpenGlFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value) {
  ...
  glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
                  Utils::HazelFBTextureFormatToGL(spec.TextureFormat), GL_INT,
                  &value);
}

uint32_t OpenGlFramebuffer::GetColorAttachmentRendererID(
    uint32_t index /*= 0*/) const {
  return m_ColorAttachments[index];
}

const Kazel::FramebufferSpecification& OpenGlFramebuffer::GetSpecification()
    const {
